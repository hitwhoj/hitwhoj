...
describe("permissionProblemCreate", () => {
  let root: Request;
  let admin: Request;
  let user: Request;
  let banned: Request;
  ...
  it("Root 可以创建题目", async () => assert(await permission.check(root)));
  it("Admin 可以创建题目", async () => assert(await permission.check(admin)));
  it("User 不可以创建题目", async () =>
    assert(!(await permission.check(user))));
  it("Banned 不可以创建题目", async () =>
    assert(!(await permission.check(banned))));
  it("Guest 不可以创建题目", async () =>
