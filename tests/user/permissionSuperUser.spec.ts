...
test("permissionSuperUser", async () => {
  ...
  assert(await permission.check(root), "Root 是超级管理员");
  assert(!(await permission.check(admin)), "Admin 不是超级管理员");
  assert(!(await permission.check(user)), "User 不是超级管理员");
  assert(!(await permission.check(banned)), "Banned 不是超级管理员");
  assert(!(await permission.check(guest)), "Guest 不是超级管理员");
});
